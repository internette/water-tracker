...
let mainWindow, winWidth = 400, winHeight = 300,
  cups = config.get('cups'), resetDate = config.get('last-reset-date'),
  notifications = config.get('notifications'), keepOnTop = config.get('keep-on-top'), sentItems = {}

const createWindow = () => {
  cups = cups === undefined ? cups : 0;
...
const checkIfMidnight = () => {
  ...
  if (hours === 0 && mins === 0 && secs >= 0 && secs <= 1) {
    ipcRenderer.send('reset-cups', '')
  }
}

// Reset all tasks on click to incomplete
ipcMain.on('reset-cups', (event, args) => {
  config.set('cups', 0);
  resetDate = new Date();
  config.set('last-reset-date', resetDate)
  sentItems = {
    cups: config.get('cups'),
    resetDate: resetDate,
    keepOnTop: keepOnTop
  }
...
ipcMain.on('get-cups', (event, args) => {
  sentItems = {
    cups: cups,
    resetDate: resetDate,
    keepOnTop: keepOnTop
...
ipcMain.on('add-cup', (event, args) => {
  sentItems.cups = sentItems.cups + 1
  config.set('cups', sentItems.cups)
  event.sender.send('send-items', sentItems)
})
